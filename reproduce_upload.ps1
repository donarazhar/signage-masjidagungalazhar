...
$start = [System.Text.Encoding]::GetEncoding('iso-8859-1').GetBytes( $bodyLines.Substring(0, $bodyLines.IndexOf($fileContent)) ) 
$end = [System.Text.Encoding]::GetEncoding('iso-8859-1').GetBytes( $bodyLines.Substring($bodyLines.IndexOf($fileContent) + $fileContent.Length) ) 
$allBytes = $start + $fileBytes + $end

try {
    Invoke-RestMethod -Uri $uploadUrl -Method Post -Headers $headers -ContentType "multipart/form-data; boundary=$boundary" -Body $bodyLines
    echo "Upload success"
} catch {
    echo "Upload failed"
